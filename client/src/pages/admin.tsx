...
import type { Search as SearchType, SearchResult, Payment } from "@shared/schema";

type AdminSearch = SearchType & {
  resultCount: number;
  isPaid: boolean;
        ...
        <Card className="w-full max-w-sm">
          <CardContent className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <p className="text-xs text-destructive text-center" data-testid="text-admin-error">{error}</p>
              )}
...
function SearchDetailDialog({
  ...
  const { data, isLoading } = useQuery<AdminSearchDetail>({
    queryKey: ["/api/admin/searches", searchId],
    enabled: !!searchId && open,
    queryFn: async () => {
      const res = await fetch(`/api/admin/searches/${searchId}`, {
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              {data.search.imageData && (
                <div className="w-20 h-20 rounded-md overflow-hidden border border-border shrink-0">
                  <img src={data.search.imageData} alt="Uploaded" className="w-full h-full object-cover" data-testid="img-admin-search-face" />
                </div>
              )}
              <div className="flex-1 min-w-0 space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Created: {new Date(data.search.createdAt).toLocaleString()}
                </p>
              </div>
            </div>
              ...
              <Card>
                <CardContent className="p-3 space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                    ...
                    <span className="font-mono truncate">{data.payment.txHash}</span>
                      ...
                      <div className="flex gap-3">
                        <div className="w-14 h-14 rounded-md overflow-hidden border border-border shrink-0">
                          <img
                            ...
                            data-testid={`img-admin-result-thumb-${i}`}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <a
                            ...
                            data-testid={`link-admin-result-${i}`}
                          >
...
function AdminDashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const { data: searches, isLoading } = useQuery<AdminSearch[]>({
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-4 h-14">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={() => window.location.href = "/"} data-testid="button-admin-home">
